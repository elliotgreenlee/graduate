...
class Classify:
    def __init__(self, Xtr_features, Xtr_classes, Xte_features, Xte_classes, prior_probability_no):
        ...
        for k in range(1, 21):
            self.k = k
            self.knn()
            new_accuracy = self.determine_accuracy(self.knn_classes)
    ...
    def decide_knn(self, sample):
        ...
        posterior_no = ((no_count * len(self.Xtr_features)) / (1.0 * self.k * len(self.Xtr_features_no))) * self.prior_no

        posterior_yes = ((yes_count * len(self.Xtr_features)) / (1.0 * self.k * len(self.Xtr_features_yes))) * self.prior_yes
